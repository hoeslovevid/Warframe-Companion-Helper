...
export function deriveCustomThemeVars(paletteInput?: Partial<CustomPalette> | null): Record<string, string> {
  ...
  return {
    ...
    '--vl-panel-border': toRgba(muted, dark ? 0.12 : 0.12),
    '--vl-panel-glow': toRgba(accentB, dark ? 0.05 : 0.05),
    '--vl-shadow': dark
      ? '0 18px 48px rgba(0, 0, 0, 0.52)'
      : `0 14px 36px ${toRgba(mix(text, muted, 0.5), 0.1)}`,
    '--vl-shadow-soft': dark
      ? '0 4px 14px rgba(0, 0, 0, 0.28)'
      : `0 4px 12px ${toRgba(mix(text, muted, 0.5), 0.06)}`,
    '--vl-scroll-track': toRgba(void950, dark ? 0.45 : 0.7),
    '--vl-scroll-thumb': toRgba(muted, 0.28),
    '--vl-scroll-thumb-hover': toRgba(accentB, 0.4),
    '--vl-bg-spot-1': toRgba(accentB, dark ? 0.08 : 0.08),
    '--vl-bg-spot-2': toRgba(accentA, dark ? 0.05 : 0.05),
    '--vl-bg-spot-3': toRgba(void850, dark ? 0.7 : 0.55),
    '--vl-bg-base-a': mix(void900, void850, 0.35),
    '--vl-bg-base-b': void900,
    '--vl-bg-base-c': void950,
    '--vl-nav-bg-a': toRgba(void950, dark ? 0.9 : 0.95),
    '--vl-nav-bg-b': toRgba(void900, dark ? 0.78 : 0.92),
    '--vl-nav-border': toRgba(muted, dark ? 0.08 : 0.1),
    '--vl-surface': toRgba(void850, dark ? 0.45 : 0.8),
    '--vl-surface-border': toRgba(muted, dark ? 0.1 : 0.12),
    '--vl-input-bg': toRgba(void950, dark ? 0.55 : 0.95),
    '--vl-input-border': toRgba(muted, dark ? 0.14 : 0.16),
    '--vl-btn-bg-a': toRgba(void800, dark ? 0.9 : 0.98),
    '--vl-btn-bg-b': toRgba(void900, dark ? 0.96 : 0.98),
    '--vl-btn-border': toRgba(muted, dark ? 0.16 : 0.14),
    '--vl-btn-primary-a': toRgba(accentB, dark ? 0.28 : 0.16),
    '--vl-btn-primary-b': toRgba(void900, dark ? 0.97 : 0.96),
    '--vl-btn-primary-border': toRgba(accentB, 0.42),
    '--vl-btn-ghost-bg': 'transparent',
    '--vl-nav-hover': toRgba(void800, dark ? 0.55 : 0.85),
    '--vl-nav-active-a': toRgba(accentB, 0.08),
    '--vl-nav-active-b': 'transparent',
    '--vl-nav-active-border': 'transparent',
    '--vl-grid-line': toRgba(muted, dark ? 0.035 : 0.04),
    '--vl-title-grad-a': text,
    '--vl-title-grad-b': text,
    '--vl-accent': accentB,
    '--vl-accent-soft': toRgba(accentB, dark ? 0.14 : 0.1),
    '--vl-premium': goldSoft,
  }
}
